...
from sklearn.decomposition import TruncatedSVD, KernelPCA, NMF
...
def load_graph(graph_path, graph_type='undirect'):
    ...
    '''
    edgelist = np.array(pd.read_csv(graph_path))
...
def reduce_dimension_svd(X, reduction_dimensions=64, svd_iterations=20, seed=42):
    '''
...
def evaluate_embeddings_nn(features, hidden, epochs, learning_rate, path_label, tr_frac, dataset='', method='', multi_label=False):
    X, Y = read_node_label_from_csv(path_label)
    print("Training classifier using {:.2f}% nodes...".format(tr_frac * 100))
    try:
